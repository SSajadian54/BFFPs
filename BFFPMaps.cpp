...
struct lens{
    int    numl,struc, con;
    double Ml, Mp, Mtot, Rl, semi,Dl, vl, vs, Vt, xls;
    double rhomaxl,tE,RE;
    double mul,u0, inc;
    double tetE, piE;
    double pt1, pt2, t0, dt; 
    double q, dis, ksi, peri; 
...
///****************************************************************************
    ...
    FILE* fil1;  
    fil1=fopen("./files/MONT/Maps/param.dat","w");
    FILE* fil2;  
    fil2=fopen("./files/MONT/Maps/MapII.dat","w");
...
///============================================================================
    ...
    read_cmd(cm,w);
    int    counter=0, li, nm=100, nv;
    double lonn0, lonn, lat0, test;
    double chiB, chiPL1, chiPL2, chiR;  
    double tim, xcent, ycent, xcent2, u1, u2, APL1, APL2, ABin; 
    double sigm, siga, flac;    
    double timw; 
    double fr, fq; 
    ...
    l.Mtot=double(-2.5+j1*5.0/nm); 
    l.Mtot=pow(10.0,l.Mtot)*Mearth;//[kg]  
    
    
    for(int i1=0; i1<nm; ++i1){ 
    l.semi=double(-2.0+i1*3.5/nm);  
    l.semi=pow(10.0,l.semi)*AU;//[m] 
    
    finter=0.0, fwide=0.0, fnum=0.0; 
    counter=0; 
    for(int icon=1; icon<=100; ++icon){
    do{
    l.q=RandR(0.0,1.0); 
    fr =RandR(0.0,1.0);  
    fq =exp(-(l.q-mu)*(l.q-mu)/(2.0*sig*sig));  
    }while(fr>fq);
    l.Ml=l.Mtot/(1.0+l.q);//[kg]
    l.Mp=l.Ml*l.q;        //[kg]
...
///######################################################################################
    /*
    sprintf(filnam1,"./files/MONT/Maps/%c%c%c%c%d.dat",'m','o','d','_', counter);
    model=fopen(filnam1,"w");
    sprintf(filnam2,"./files/MONT/Maps/%c%c%c%c%d.dat",'d','a','t','_', counter);
    data= fopen(filnam2,"w");
    timw=0.0; 
    chiB=0.0, chiPL1=0.0, chiPL2=0.0, chiR=0.0;
    l.pt1=double(-3.5*l.tE+l.t0);//days  
    ...
    APL1=s.blend[4]*APL1+1.0-s.blend[4];
    APL2=s.blend[4]*APL2+1.0-s.blend[4];
    ABin=s.blend[4]*ABin+1.0-s.blend[4];
    if(APL1<1.0 or APL2<1.0 or ABin<1.0 or s.blend[4]<0.0 or s.blend[4]>1.0 or s.ros<0.0 or l.dis<0.0 or 
    l.q>1.0 or l.q<0.0 or l.dis>10.0){
    cout<<"ERoRR APL1: "<<APL1<<"\t APL2:  "<<APL2<<"\t ABin:  "<<ABin<<endl;
    cout<<"blend[4]: "<<s.blend[4]<<"\t Rho_*:  "<<s.ros<<"\t Dis:  "<<l.dis<<endl;
    cout<<"q:  "<<l.q<<"\t xcent: "<<xcent<<"\t ycent:  "<<ycent<<endl;
    ...
    chiPL2+= (ABin+flac-APL2)*(ABin+flac-APL2)/siga/siga;//ESPL2 chi2
    chiR  += (ABin+flac-ABin)*(ABin+flac-ABin)/siga/siga;///real_model chi2
    chiB  += (ABin+flac-1.00)*(ABin+flac-1.00)/siga/siga;///Baseline chi2
    fprintf(data,"%.4lf    %.6lf   %.6lf\n",tim, ABin+flac, siga);}}}
    fclose(data);
    fclose(model); */
///######################################################################################   
    //fil1=fopen("./files/MONT/Maps/Map1.dat","a+");
    fprintf(fil1,
    "%d  %.5lf  %.5lf  "//3
    "%d  %.8lf   %.7lf   %.7lf  %.5lf  %.5lf  %.5lf  "//10
    "%d  %d  %.7lf   %.7lf  %.7lf  %.7lf  %.4lf  %.2lf   %.4lf  %.4lf   %.4lf  "//21
    "%.5lf  %.8lf  %.5lf   %.7lf  %d "  //26
    "%.8lf  %.9lf  %.7lf   %.8lf  %.8lf  %.5lf  %.9lf  %.9lf  %.6lf  %.8lf  "//36
    "%.5lf  %.5lf %.5lf  %.5lf  %.5lf  %.5lf %.5lf %.5lf %.2lf %.2lf  %.4lf  %.4lf  %.6lf  %.5lf  %.5lf  %.5lf  %.5lf %.5lf %d %d\n",
    counter, double(-2.5+j1*5.0/nm) , double(-2.0+i1*3.5/nm) , //3
    l.struc, l.Ml/Mearth, l.Rl/Rearth, l.Dl, l.vl, s.mul1, s.mul2, //10
    s.struc, s.cl, s.mass, s.Ds, s.Tstar, s.Rstar, s.logl, s.type, l.vs, s.mus1, s.mus2,//21
    l.q, l.dis, log10(l.peri), log10(l.semi/AU), l.con, //26
    l.tE, l.RE/AU, l.mul*30.0, l.Vt, l.u0, l.ksi*180.0/M_PI, l.piE, l.tetE,s.opd*1.0e6, s.ros,///36
    s.Mab[1],s.Mab[4],s.Map[1],s.Map[4],s.magb[1],s.magb[4],s.blend[1],s.blend[4],s.Nblend[1],s.Nblend[4],s.Ai[1],s.Ai[4],
    l.t0,chiPL1, chiPL2, chiR, chiB, l.inc,i1,j1);///54+2

    if(int(counter)%30==0){
    cout<<"============================================================="<<endl;
    cout<<"counter:     "<<counter<<"\t icon:  "<<icon<<endl;
    cout<<"Chir_real:  "<<chiR<<"\t Chi_Baseline: "<<chiB<<endl;
    cout<<"chi_PL2:    "<<chiPL2<<"\t Chi2_PL1:  "<<chiPL1<<endl;
    ...
    finter=double(finter*100.0/fnum); 
    fwide =double( fwide*100.0/fnum);
    fprintf(fil2,"%d   %d   %.7lf   %.7lf\n",i1,j1, finter, fwide);}}
    fclose(fil2); 
    fclose(fil1);
    return(0);
}
...
///&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
double ErrorRoman(lens & l, double ghadr){

     double error=-1.0, shib=0.0; 

     if(ghadr<l.magw[0] or ghadr==l.magw[0]) error= l.errw[0];
     
     else if(ghadr>l.magw[Nw-1] or ghadr==l.magw[Nw-1]) {
     shib=(l.errw[Nw-1]-l.errw[Nw-2])/(l.magw[Nw-1]-l.magw[Nw-2]); 
     error=l.errw[Nw-1]+shib*(ghadr-l.magw[Nw-1]);      }
     
     else{
     for(int i=1; i<Nw; ++i){
     if(double((ghadr-l.magw[i])*(ghadr-l.magw[i-1]))<0.0 or  ghadr==l.magw[i-1]){
     shib=(l.errw[i]-l.errw[i-1])/(l.magw[i]-l.magw[i-1]); 
     error=l.errw[i-1]+shib*(ghadr-l.magw[i-1]); 
     break;}}}
     
     if(error<0.001 or error>0.85 or ghadr>26.0 or ghadr<0.0){
     cout<<"ERROR error: "<<error<<"\t ghadr: "<<ghadr<<endl; int uue; cin>>uue;  }
     return(error);
}
///==============================================================//6
///                                                              //
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
    ...
    s.Fluxb[i]<-0.009543 or s.Ai[i]<0.0 or s.magb[i]<0.0 or s.magb[i]>100.0){
    cout<<"BIGG ERRROR nsbl: "<<s.Nblend[i]<<"\t Nblend[i]: "<<s.Nblend[i]<<"\t belnd: "<<s.blend[i]<<endl;
    cout<<"BIG ERROR Flux is negative: "<<s.Fluxb[i]<<"\t No. filter:  "<<i<<"\t ext:  "<<s.Ai[i]<<endl; 
    cout<<"s.magb[i]:   "<<s.magb[i]<<endl;    cin>>yye;} }
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH   
    ...
    min1=1000000000.0;
    for(int w1=0; w1<800; ++w1){
    dc= double(0.5+w1*(0.5/800.0)); 
    ...
    cout<<"Error dwide:    "<<dwide<<"\t dclose:  "<<dclos<<"\t   semi:  "<<l.semi/l.Rl<<endl;
...
////=================================== THIN DISK ==============================
    ...
    or cm.mass_d[j]==0.0 or cm.Teff_d[j]<0 or metal>0.1 or age>10 or int(cm.cl_d[j])==6 or float(cm.type_d[j])>8 or cm.type_d[j]<1.0){
