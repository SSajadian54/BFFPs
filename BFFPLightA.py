...
for im in range(1):
    ...
    for k in range(num):#MonteCarlo  
        ...
        parm[:nc]=par[i,:]
        if(abs(im-Massi)>0.2 or abs(LMtot-float(-2.5+im*5.0/10))>0.01 or Ml<0.0 or Dl>Ds or q<0.0 or dis<0.0 or int(ei-ini-1)!=i or fb>1.0 or t0>Tobs 
        or t0<0.0 or tE<0.0 or tE>120.0 or ChiB<0.0 or ChiR<0.0 or ChiPL1<0.0 or ChiPL2<0.0  or config<0.9 or config>3.1 or 
          abs(LMtot+2.5)>0.1): 
            ...
            print("Chis: ",  ChiB, ChiR, ChiPL1, ChiPL2, config, ei, i)
        ...
        #######################################################################             
        if(flagd>0 and nd>2 and nm>10 and ei>3913):
            dat=np.zeros((nd,3))
            mod=np.zeros((nm,6))
