...
nam0=["count", "Massi", "Mtot(Mearth)", "strucL", "Ml(Mearth)", "Dl(Kpc)", "vl(km/s)", "mul1", "mul2", "strucS", "cl", "mass(Msun)", "Ds(kpc)", "Tstar(K)", "Rstar(Rsun)", "logl", "typeS", "vs(km/s)", "mus1", "mus2", "q", "log10[dis]", "log10(period)", "log10(Semi/AU)", "log10[tE(days)]", "RE/AU", "mul*30", "Vt(km/s)", "u0", "ksi(degree)", "log10[pi_rel]", "tetE(mas)", "opd*1.0e6", "log10[ros]" ,"Mab1","Mab4","Map1","Map4","magb1","magb4","blend1","blend4","Nblend1","Nblend4", "Ext1","Ext4","t0", "ChiPL1","ChiPL2", "ChiReal", "ChiBaseline", "inclination", "Configuration", "Numsemi"]
...
ny=3000
cau=np.zeros((ny, nx))
angle =np.linspace(0.0 , 2*np.pi , 150 ) 
xsou  =np.cos(angle) 
ysou  =np.sin(angle)
ndim  =5
Nwalkers=60
...
num=1000 ## number of semi major axis
nf =14*num
nc =54
parm=np.zeros((nc+ndim*3+2))
par = np.zeros((nf,nc))
par = np.loadtxt("./param3.dat")   
...
def prior(p, prt): 
    ...
    if(u01>0.0 and u01<=1.5 and t01>=0.0 and t01<=Tobs and tE1>0.0 and tE1<160.0 and fb1>0.0 and fb1<=1.0 and rho1>=0.0 and 
        rho1<100.0 and abs(u00-u01)<0.8 and abs(t01-t00)<=float(tE0*1.5) and abs(tE1-tE0)<=float(tE0*1.5) and abs(fb1-fb0)<0.8 and 
        abs(rho1-rho0)<=float(10.0*rho0)): 
        return(0); 
    return(-1.0);     
#======================================    
...
def lnlike2( p, tim, Magni, erra):
    ...
    for ji in range(len(tim)):  
        ...
        chii+=(fb1*As+1.0-fb1-Magni[ji])**2.0/(erra[ji]*erra[ji])
    return(-1.0*chii)


###############################################################################
'''
detec= np.zeros((nf,nc)); k=0
for i in range(nf):  
    if(abs(par[i,49]-par[i,50])>=100.0):
        detec[k,:]=par[i,:]
        k+=1     
print ("Number of detected lensing events:  ",  k, float(k*100.0/nf) )        
for i in range(nc):
    plt.cla()
    plt.clf()
    fig=plt.figure(figsize=(8,6))
    ax =plt.gca()              
    if(i!=21 and i!=33 and i!=30 and i!=24):
        plt.hist(par[:,i],30,histtype='bar',ec='darkgreen',facecolor='green',alpha=0.5, rwidth=1.5, label="Simulated Microlensing")
        plt.hist(detec[:k,i],30,histtype='bar',ec='darkred',facecolor='red',alpha=0.5, rwidth=1.5, label="Detectable Microlensing")
    
    if(i==21 or i==33 or i==24):
        plt.hist(np.log10(par[:,i]),30,histtype='bar',ec='darkgreen',facecolor='green',alpha=0.5,rwidth=1.5, label="Simulated Microlensing")
        plt.hist(np.log10(detec[:k,i]),30,histtype='bar',ec='darkred',facecolor='red',alpha=0.5,rwidth=1.5, label="Detectable Microlensing")
        
    if(i==30):
        plt.hist(np.log10(par[:,i]*par[:,31]),30,histtype='bar',ec='darkgreen',facecolor='green',alpha=0.5,rwidth=1.5, label="Simulated Microlensing")
        plt.hist(np.log10(detec[:k,i]*detec[:k,31]),30,histtype='bar',ec='darkred',facecolor='red',alpha=0.5,rwidth=1.5, label="Detectable Microlensing")
    ...
    fig=plt.gcf()
    fig.savefig("./fig/hist{0:d}.jpg".format(i),dpi=200)
...
###############################################################################
mtot=np.array([-1.75, -1.5, -1.25, -1.0, -0.75, -0.5, -0.25, 0.0, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0])
im=0
...
for k in range(num):#Semi-major axis
    i+=1
    
    ei,counM,LMtot, strucl, Ml,Dl       =int(par[i,0]), int(par[i,1]), par[i,2], par[i,3], par[i,4], par[i,5]  
    ...
    ChiPL2, ChiR, ChiB,inc,config,nsem =par[i,48], par[i,49], par[i,50], par[i,51], int(par[i,52]), int(par[i,53])
    parm[:nc]=par[i,:]
    
    if(abs(im-counM)>0.2 or abs(LMtot-mtot[im])>0.01 or Ml<0.0 or Dl>Ds or q<0.0 or dis<0.0 or int(ei-1)!=i or fb>1.0 or t0>Tobs 
        or t0<0.0 or tE<0.0 or tE>120.0 or ChiB<0.0 or ChiR<0.0 or ChiPL1<0.0 or ChiPL2<0.0 or config<0.9 or config>3.1 or 
        nsem!=int(k+1)): 
            print("Error:  ", im+1, counM, LMtot, Ml, Dl, Ds, q, dis, ei, fb, t0, Tobs, tE)
            print("Chis: ",  ChiB, ChiR, ChiPL1, ChiPL2, config, ei, i, k)
    ...
    DCHIL= abs(ChiR-ChiB)
    Mtot=pow(10.0,LMtot)#in earth mass
    ...
    #######################################################################             
    if(flagd>0 and nd>2 and nm>10 and ei>966):
        dat=np.zeros((nd,3))
        mod=np.zeros((nm,6))
        ...
        sampler=emcee.EnsembleSampler(Nwalkers,ndim,likelihood,args=(tim,Magni,erra,prt), threads=2)
        ...
        Chif=abs(lnlike2(Chain[tt,:],tim, Magni, erra))
        ffd=  abs(chi2s[tt])
        print("Chi2s, argmin,  param_min, chis: ",chi2s, tt, Chain[tt,:], ffd, Chif)
        if(abs(Chif-ffd)>0.1):  
            print("BIG ERROR: chi2s:  ",  Chif, ffd)
        ...
        parm[nc:]=np.array([u0f,pb[0,1],pb[0,2],t0f,pb[1,1],pb[1,2],tEf,pb[2,1],pb[2,2],fbf,pb[3,1],pb[3,2],rosf,pb[4,1],pb[4,2],Chif,ei])
        save2=open("./ParamLast{0:d}.txt".format(im),"a")
        np.savetxt(save2,parm.reshape(-1,nc+17),fmt='%d  %d   %.5f  %d  %.8f  %.7f  %.5f  %.5f  %.5f  %d  %d  %.7f   %.7f  %.7f  %.7f  %.4f  %.2f  %.4f  %.4f  %.4f  %.5f  %.8f  %.5f   %.7f %.8f  %.9f  %.7f   %.8f %.6f  %.5f  %.5f  %.9f  %.6f  %.6f  %.5f  %.5f  %.5f  %.5f  %.5f  %.5f  %.5f %.5f %.2f  %.2f  %.4f  %.4f  %.6f  %.5f  %.5f  %.5f  %.5f  %.5f  %d %d %.6f  %.6f  %.6f %.6f  %.6f  %.6f %.6f  %.6f %.6f %.6f  %.6f  %.6f  %.8f  %.8f  %.8f  %.5f  %d')
        save2.close()
        mfit=np.zeros((nm))
        mfda=np.zeros((nd))
        ...
        ax1.plot(mod[:,0],mfit,   'g-.' ,label=r"$\rm{ESPL}~\rm{fitted}~\rm{model}$",lw=1.7)
        ax1.plot(mod[:,0],mod[:,3],'k-', label=r"$\rm{ESBL}~\rm{real}~\rm{model}$",lw=1.7)
        ax1.errorbar(dat[:,0],dat[:,1],yerr=dat[:,2],fmt=".",markersize=8.0,color='m',ecolor='gray',elinewidth=0.5, capsize=0, alpha=0.8)
        ...
        r"$,~\rm{q}=$"+str(round(q,1))+
        r"$,~\log_{10}[d]=$"+str(round(np.log10(dis),2))+
        r"$,~t_{\rm{E},t}\rm{(days)}=$"+str(round(tE,2))+
        "\n"+r"$u_{0,t}=$"+str(round(u0,1))+r"$,~\rho_{\star,t}=$"+str(round(ros,2))+
        r"$,~u_{0,b}=$"+str(round(u0f,1))+r"$\pm$"+str(round(du0,1))+
        r"$,~t_{\rm{E},b}\rm{(days)}=$"+str(round(tEf,2))+r"$\pm$"+str(round(dtE,2))+
        r"$,~\rho_{\star,b}=$"+str(round(rosf,2))+r"$\pm$"+str(round(dro,1))+
        r"$,~\Delta\chi^{2}_{\rm{B}}=$"+str(int(abs(Chif-ChiR))),fontsize=14.5,color='k')
        ...
        plt.legend()
        plt.legend(loc=1,fancybox=True, fontsize=14, framealpha=0.5)
        ###################################################################
        left, bottom, width, height=[0.09, 0.5, 0.33, 0.33]
        ...
        ax2.errorbar(dat[:,0],dat[:,1]-mfda,yerr=dat[:,2],fmt=".",markersize=10.8,color='m',ecolor='gray',elinewidth=0.8,capsize=0, alpha=0.8) 
        ax2.plot(mod[:,0],    mod[:,3]-mfit ,"k-", lw=1.7)
        plt.xticks(fontsize=17, rotation=0)
        plt.yticks(fontsize=17, rotation=0)
        ...
        plt.subplots_adjust()
        fig=plt.gcf()
        fig.savefig("./fig/LC{0:d}_{1:d}.jpg".format(int(im), int(k)), dpi=200)
        #######################################################################
